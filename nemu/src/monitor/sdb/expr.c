...
static struct rule {
  ...
  {"\\$pc|\\$0|ra|sp|gp|tp|t0|t1|t2|s0|s1|a0|a1|a2|a3|a4|a5|a6|a7|s2|s3|s4|s5|s6|s7|s8|s9|s10|s11|t3|t4|t5|t6", TK_REG},
  {"\\+", '+'},         // plus
  {"\\*", '*'},         // time || dereferencels
  {"\\-", '-'},         // minus
  {"\\/", '/'},         // divide
  ...
  if (p > q) {
    ...
    *success = false;
    ...
    if (p->type == DEREF && (q->type == TK_REG || q->type == TK_NUM)) {
      ...
      switch(p->type) {
        case TK_POS:
          if (strncmp("0x", q->str, 2) == 0) ret = strtol(q->str, NULL, 16);
          else ret = strtol(q->str, NULL, 10);
          break;
        case TK_NEG:
          if (strncmp("0x", q->str, 2) == 0) ret = -strtol(q->str, NULL, 16);
          else ret = -strtol(q->str, NULL, 10);
