...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , UJ, R(rd) = s->pc + 4; s->dnpc = s->pc + imm; IFDEF(CONFIG_FTRACE, {
            if (rd == 1) {
  ...
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , SB, if ((sword_t)src1 <  (sword_t)src2) s->dnpc = s->pc + imm);
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , SB, if ((sword_t)src1 >= (sword_t)src2) s->dnpc = s->pc + imm);
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti   , I , R(rd) = ((sword_t)src1 < SEXT(imm, 32))?1 : 0);
