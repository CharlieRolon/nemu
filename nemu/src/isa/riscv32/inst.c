...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , SB, if ((int)src1 <  (int)src2) s->dnpc = s->pc + imm);
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , SB, if ((int)src1 >= (int)src2) s->dnpc = s->pc + imm);
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I , R(rd) = (word_t)SEXT(Mr(src1 + imm, 4), 32));
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I , R(rd) = (int)src1 >> BITS(imm, 4, 0));
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R , R(rd) = ((int64_t)src1 *  (int64_t)src2) >> sizeof(word_t) * 8);
